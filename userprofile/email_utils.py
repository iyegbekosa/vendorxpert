...
def send_receipt_email(order):
    ...
    try:
        ...
        order_items = OrderItem.objects.filter(order=order).select_related('product', 'product__vendor')
        ...
        for item in order_items:
            ...
            items_data.append({
                ...
                'product_image': item.product.get_thumbnail() if hasattr(item.product, 'get_thumbnail') else None,
            })
        ...
        context = {
            ...
            'pickup_location': dict(order.PICKUP_CHOICES).get(order.pickup_location, order.pickup_location),
        ...
        if result:
            logger.info(f"Receipt email sent successfully to {user.email} for order {order.ref}")
            return True
        else:
            logger.error(f"Failed to send receipt email to {user.email} for order {order.ref}")
            return False
...
def send_vendor_order_notification(order):
    ...
    try:
        ...
        order_items = OrderItem.objects.filter(order=order).select_related('product', 'product__vendor', 'product__vendor__user')
        ...
        for vendor, items in vendor_orders.items():
            try:
                ...
                vendor_total = sum(item.price * item.quantity for item in items) / 100  # Convert from kobo to naira
                ...
                # Render HTML and text templates
                html_content = render_to_string('emails/vendor_order_notification.html', context)
                text_content = render_to_string('emails/vendor_order_notification.txt', context)
                ...
                if result:
                    logger.info(f"Vendor notification email sent to {vendor.user.email} for order {order.ref}")
                else:
                    logger.error(f"Failed to send vendor notification to {vendor.user.email} for order {order.ref}")
                    all_emails_sent = False
                    
            except Exception as e:
                logger.error(f"Error sending vendor notification to {vendor.store_name}: {str(e)}")
                all_emails_sent = False
        
        return all_emails_sent
        
    except Exception as e:
        logger.error(f"Error sending vendor notifications for order {order.ref}: {str(e)}")
        return False